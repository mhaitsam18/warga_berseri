    ...
    <div class="row">
      <div class="col-lg-12 text-center">
          <ul class="list-inline mb-0">
           ...
           <h1 class="text-lg text-white mt-2">Peraturan / Hasil Rapat</h1>
      </div>
    </div>
  </div>
</section>
			...
			<?php foreach ($peraturan as $row): ?>
				<div class="col-lg-4 col-md-6">
					<article class="card border-0 rounded-0 mb-4">
						<img src="<?= base_url2('assets/img/peraturan/').$row['thumbnail'] ?>" alt="<?=$row['thumbnail']?>" class="img-fluid">
						<div class="mt-3 px-4 py-3 ">
							...
							<a href="<?= base_url('Home/detailPeraturan/').$row['id'] ?>"><h4 class="mb-3 font-secondary"><?= $row['judul'] ?></h4></a>
							<?php 
							...
							if ($jml_karakter>50) {
							 ...
							 ?>
							<p class="mb-4"><?= $isi ?>...</p>
							<a href="<?= base_url('Home/detailPeraturan/').$row['id'] ?>" class="text-color mb-3 d-block"><i class="ti-minus mr-2"></i> Read More</a>
							<!-- stretched-link -->
						</div>
					</article>
				</div>
